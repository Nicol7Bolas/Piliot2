...
function f_condition_distance($cti,$data)
{
    ...
    if ($data->power_consumption != "") {
        ...
        if ($cti[$comp-1] == 1) {
            $conditions = $conditions.' AND emission_power <= '.$data->power_consumption.''; $used += $data->power_consumptionI;
        }
        else { $error[count($error)] = "emission_power"; }
    }
