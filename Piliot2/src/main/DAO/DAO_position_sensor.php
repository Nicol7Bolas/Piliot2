...
function f_condition_position($cti,$data)
{
    $conditions = ""; $used = 0; $terms = 0; $comp = 0; $error = array();
    if ($data->range_min != "") {
        $terms += $data->rangeI; $comp++;
        if ($cti[$comp-1] == 1) {
            $conditions = $conditions.' AND `range_min` <= '.$data->range.''; $used += $data->rangeI;
        }
        else { $error[count($error)] = "range_min"; }
    }
    if ($data->range_max != "") {
        $terms += $data->rangeI; $comp++;
        if ($cti[$comp-1] == 1) {
            $conditions = $conditions.' AND `range_max` >= '.$data->range.''; $used += $data->rangeI;
        }
        else { $error[count($error)] = "range_max"; }
    }
    ...
    if ($data->max_lighting != "") {
        ...
        if ($cti[$comp-1] == 1) {
            $conditions = $conditions.' AND maximum_luminosity <= '.$data->max_lighting.''; $used += $data->max_lightingI;
        }
        else { $error[count($error)] = "maximum_luminosity"; }
    }
